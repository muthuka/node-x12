...
import { errorLookup, X12Interchange, X12Parser, X12Segment, X12ValidationEngine, X12ValidationErrorCode, } from "../mod.js";
import { X12ElementRule, X12GroupRule, X12InterchangeRule, X12SegmentRule, X12TransactionRule, X12ValidationRule, } from "../src/X12ValidationEngine/index.js";
...
describe("X12ValidationEngine", () => {
    ...
    it("should validate X12 document", () => {
        ...
        let rule = new X12InterchangeRule(ruleJson);
        ...
        assert.strictEqual(report, true);
        rule = new X12ElementRule(ruleJson.group.transaction.segments[0].elements[0]);
    ...
    it("should resolve error codes", () => {
        ...
        for (const errorType of errorTypes) {
            for (let i = 1, j = 1; i <= j; i += 1) {
                const result = errorLookup(errorType, j.toString());
