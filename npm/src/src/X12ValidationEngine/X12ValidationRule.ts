...
import { errorLookup } from "./X12ValidationErrorCode.js";
  ...
  assert?(value: any, position?: number): true | ValidationReport {
    ...
    if (this.engine instanceof RegExp) {
      if (!this.engine.test(`${value}`)) {
        errors.push(
          errorLookup(this.ruleType as any, this.failureCode, position, value),
        );
      }
    }
...
export class X12InterchangeRule extends X12ValidationRule {
  constructor(options: X12InterchangeRule) {
    ...
    if (
      options.header instanceof X12SegmentRule ||
    ...
    if (
      options.trailer instanceof X12SegmentRule ||
  ...
  assert?(interchange: X12Interchange): true | ValidationReport {
    ...
    const groupNumber = parseFloat(
      interchange.functionalGroups[0].header.valueOf(6, "0") ?? "0",
    );
    const groupReport =
      this.group.assert?.(interchange.functionalGroups[0], groupNumber) ?? {};
...
export class X12GroupRule extends X12ValidationRule {
  ...
  assert?(
    ...
    if (headerResult !== true) {
      const groupIdResult = headerResult?.elements?.find((error) =>
        error.position === 1
      );
      const groupNumberResult = headerResult?.elements?.find((error) =>
        error.position === 6
      );
    ...
    for (const transaction of group.transactions) {
      const transactionNumber = parseFloat(
        transaction.header.valueOf(2, "0") ?? "0",
      );
    ...
    if (trailerResult === true) {
      ...
      const trailerControlNumber = parseFloat(
        group.trailer.valueOf(2, "0") ?? "0",
      );
...
export class X12TransactionRule extends X12ValidationRule {
  constructor(options: X12TransactionRule) {
    ...
    this.segments = options.segments.map((segmentRule) =>
      new X12SegmentRule(segmentRule)
    );
  ...
  assert?(
    ...
    const handleLoop = function handleLoop(
      ...
      for (
        let i = index, loopPosition = 0;
        i < transaction.segments.length;
        i += 1
      ) {
    ...
    if (headerResult !== true) {
      const transactionIdResult = headerResult?.elements?.find((error) =>
        error.position === 1
      );
      const transactionNumberResult = headerResult?.elements?.find((error) =>
        error.position === 2
      );
    ...
    const trailerResult = this.trailer.assert?.(
      transaction.trailer,
      transaction.segments.length + 2,
    );

    if (trailerResult === true) {
      const segmentCount = parseFloat(
        transaction.trailer.valueOf(1, "0") ?? "0",
      );
      const trailerControlNumber = parseFloat(
        transaction.trailer.valueOf(2, "0") ?? "0",
      );
...
export class X12SegmentRule extends X12ValidationRule {
  ...
  assert?(segment: X12Segment, position = 1): true | ValidationReport {
    ...
    if (segment.tag !== this.tag) {
      ...
      if (this.mandatory) {
        errors.push(errorLookup(this.ruleType, "3", position));
      }
    }
