...
export class X12ValidationEngine {
  ...
  private setAcknowledgement(
    ...
    if (isa instanceof X12Segment && gs instanceof X12Segment) {
      ...
      this.acknowledgement.setHeader(
        isa.elements.map((element) => element.value),
      );
      this.acknowledgement.addFunctionalGroup().setHeader(
        gs.elements.map((element) => element.value),
      );
    }
  }
  ...
  assert(
    actual: X12Transaction,
    expected: X12TransactionRule,
  ...
  assert(
    actual: any,
    expected: X12ValidationRule,
    _groupResponse?: GroupResponseCode,
  ): true | ValidationReport {
    ...
    if (
      actual instanceof X12Interchange && expected instanceof X12InterchangeRule
    ) {
      const groupId = actual.functionalGroups[0].header.valueOf(1) ?? "";
      const groupNumber = parseFloat(
        actual.functionalGroups[0].header.valueOf(6, "0") ?? "0",
      );
    ...
    if (
      actual instanceof X12FunctionalGroup && expected instanceof X12GroupRule
    ) {
    ...
    if (
      actual instanceof X12Transaction && expected instanceof X12TransactionRule
    ) {
      const transactionNumber = parseFloat(
        actual.header.valueOf(2, "0") ?? "0",
      );
    ...
    if (this.throwError && !this.pass) {
      throw new ValidationEngineError(
        "The actual X12 document did not meet the expected validation.",
        this.report ?? {},
      );
    }
