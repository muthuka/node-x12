...
import { ValidationError, ValidationType } from "./Interfaces.js";
...
export const X12ValidationErrorCode: Record<
  string,
  (...args: any[]) => ValidationError
> = {
  element(
    code: string,
    position?: number,
...
export function errorLookup(
  ...
  return X12ValidationErrorCode[codeType ?? "segment"](
    code,
    position,
    dataSample,
  );
