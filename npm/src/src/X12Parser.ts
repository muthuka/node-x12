...
} from "./X12SerializationOptions.js";
...
export class X12Parser extends Transform {
  ...
  getInterchangeFromSegments(
    ...
    this._options = options === undefined
      ? this._options
      : defaultSerializationOptions(options) as any;
  ...
  private _validateEdiLength(edi: string): void {
    if (edi.length < DOCUMENT_MIN_LENGTH) {
      const errorMessage =
        `X12 Standard: Document is too short. Document must be at least ${DOCUMENT_MIN_LENGTH} characters long to be well-formed X12.`;
  ...
  private _parseSegments(
    ...
    for (let i = 0, l = 0, c = 0; i < edi.length; i++) {
      ...
      if (
        !tagged && edi[i].search(/\s/) === -1 && edi[i] !== elementDelimiter &&
        ...
        if (
          currentSegment.tag === "IEA" && currentSegment.elements.length === 2
        ) {
          currentSegment.elements[1].value = `${
            parseInt(currentSegment.elements[1].value, 10)
          }`;
        }
        ...
        if (
          currentSegment.tag === "ISA" && currentSegment.elements.length === 13
        ) {
          currentSegment.elements[12].value = `${
            parseInt(currentSegment.elements[12].value, 10)
          }`;
        }
  ...
  private _processSegment(seg: X12Segment): void {
    if (seg.tag === "ISA") {
      if (
        this._strict && this._interchange !== undefined &&
      ...
      if (this._group === undefined) {
        ...
        this.diagnostics.push(
          new X12Diagnostic(X12DiagnosticLevel.Error, errorMessage, seg.range),
        );
      }
      ...
      if (this._group === undefined) {
        const errorMessage =
          `X12 Standard: ${seg.tag} segment cannot appear outside of a functional group.`;
      ...
      if (this._transaction === undefined) {
        const errorMessage =
          `X12 Standard: ${seg.tag} segment cannot appear outside of a transaction.`;
  ...
  private _processIEA(interchange: X12Interchange, segment: X12Segment): void {
    ...
    if (
      parseInt(segment.valueOf(1) ?? "") !== interchange.functionalGroups.length
    ) {
      const errorMessage = `X12 Standard: The value in IEA01 (${
        segment.valueOf(
          1,
        )
    ...
    if (segment.valueOf(2) !== interchange.header.valueOf(13)) {
      const errorMessage = `X12 Standard: The value in IEA02 (${
        segment.valueOf(
          2,
        )
      }) does not match the value in ISA13 (${
        interchange.header.valueOf(13)
      }).`;
  ...
  private _processGE(group: X12FunctionalGroup, segment: X12Segment): void {
    ...
    if (parseInt(segment.valueOf(1) ?? "") !== group.transactions.length) {
      const errorMessage = `X12 Standard: The value in GE01 (${
        segment.valueOf(
          1,
        )
    ...
    if (segment.valueOf(2) !== group.header.valueOf(6)) {
      const errorMessage = `X12 Standard: The value in GE02 (${
        segment.valueOf(
          2,
        )
  ...
  private _processSE(transaction: X12Transaction, segment: X12Segment): void {
    ...
    if (parseInt(segment.valueOf(1) ?? "") !== expectedNumberOfSegments) {
      const errorMessage = `X12 Standard: The value in SE01 (${
        segment.valueOf(
          1,
        )
    ...
    if (segment.valueOf(2) !== transaction.header.valueOf(2)) {
      const errorMessage = `X12 Standard: The value in SE02 (${
        segment.valueOf(
          2,
        )
  ...
  private _consumeChunk(chunk: string): void {
    ...
    if (this._parsedISA) {
      ...
      if (
        chunk.charAt(chunk.length - 1) !== this._options.segmentTerminator &&
        !this._flushing
      ) {
        this._dataCache = rawSegments[rawSegments.length - 1];
        rawSegments.splice(rawSegments.length - 1, 1);
