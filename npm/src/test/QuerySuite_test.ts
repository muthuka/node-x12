...
describe("X12QueryEngine", () => {
  ...
  it("should handle FOREACH macro references", () => {
    ...
    if (result?.values.length !== 6) {
      throw new Error(
        `Expected six matching elements for FOREACH(PO1)=>PID05:PID01["F"]; received ${
  ...
  it("should return valid range information for segments and elements", () => {
    ...
    if (result?.segment?.range.start.line !== 3) {
      throw new Error(
        `Start line for segment is incorrect; found ${
          result?.segment?.range.start.line
        }, expected 3.`,
      );
    }
    ...
    if (result?.element?.range.start.line !== 3) {
      throw new Error(
        `Start line for element is incorrect; found ${
