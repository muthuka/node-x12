...
const X12Diagnostic_js_1 = require("./X12Diagnostic.js");
const X12FatInterchange_js_1 = require("./X12FatInterchange.js");
...
class X12Parser extends deps_js_1.Transform {
    /**
    ...
    constructor(strict, encoding, options) {
        ...
        this._decoder = new deps_js_1.StringDecoder(encoding);
    ...
    parse(edi, options) {
        if (edi === undefined) {
            throw new Errors_js_1.ArgumentNullError("edi");
        }
    ...
    _validateEdiSegmentCount() {
        ...
        if (this._strict) {
            throw new Errors_js_1.ParserError(errorMessage);
        }
        this.diagnostics.push(new X12Diagnostic_js_1.X12Diagnostic(X12Diagnostic_js_1.X12DiagnosticLevel.Error, errorMessage, new Positioning_js_1.Range(0, 0, 0, 1)));
    }
    _validateEdiLength(edi) {
        if (edi.length < DOCUMENT_MIN_LENGTH) {
            ...
            this.diagnostics.push(new X12Diagnostic_js_1.X12Diagnostic(X12Diagnostic_js_1.X12DiagnosticLevel.Error, errorMessage, new Positioning_js_1.Range(0, 0, 0, edi.length - 1)));
        }
    }
    _validateIsaLength(edi, elementDelimiter) {
        if (edi.charAt(103) !== elementDelimiter) {
            ...
            this.diagnostics.push(new X12Diagnostic_js_1.X12Diagnostic(X12Diagnostic_js_1.X12DiagnosticLevel.Error, errorMessage, new Positioning_js_1.Range(0, 0, 0, 2)));
        }
    }
    _detectOptions(edi, options) {
        ...
        if (options === undefined) {
            ...
            this._options = (0, X12SerializationOptions_js_1.defaultSerializationOptions)({
                segmentTerminator,
                elementDelimiter,
                subElementDelimiter,
                repetitionDelimiter,
            ...
            this._options = (0, X12SerializationOptions_js_1.defaultSerializationOptions)(options);
        }
    }
    _parseSegments(edi, segmentTerminator, elementDelimiter) {
        const segments = new Array();
        let tagged = false;
        let currentSegment = new X12Segment_js_1.X12Segment();
        let currentElement = new X12Element_js_1.X12Element();
        for (let i = 0, l = 0, c = 0; i < edi.length; i++) {
            ...
            if (!tagged && edi[i].search(/\s/) === -1 && edi[i] !== elementDelimiter &&
                ...
                if (currentSegment.range.start === undefined) {
                    currentSegment.range.start = new Positioning_js_1.Position(l, c);
                }
                ...
                tagged = true;
                currentElement = new X12Element_js_1.X12Element();
                currentElement.range.start = new Positioning_js_1.Position(l, c);
                // segment terminator
            }
            else if (edi[i] === segmentTerminator) {
                currentElement.range.end = new Positioning_js_1.Position(l, c - 1);
                ...
                currentSegment.range.end = new Positioning_js_1.Position(l, c);
                segments.push(currentSegment);
                currentSegment = new X12Segment_js_1.X12Segment();
                tagged = false;
                ...
                currentElement = new X12Element_js_1.X12Element();
                currentElement.range.start = new Positioning_js_1.Position(l, c + 1);
                // element data
            }
            else {
        ...
        if (seg.tag === "ISA") {
            if (this._strict && this._interchange !== undefined &&
                ...
                if (this._fatInterchange === undefined) {
                    this._fatInterchange = new X12FatInterchange_js_1.X12FatInterchange(this._options);
                    this._fatInterchange.interchanges.push(this._interchange);
                }
                this._interchange = new X12Interchange_js_1.X12Interchange(this._options);
            }
            ...
            this._group = new X12FunctionalGroup_js_1.X12FunctionalGroup(this._options);
            ...
            if (this._group === undefined) {
                ...
                this.diagnostics.push(new X12Diagnostic_js_1.X12Diagnostic(X12Diagnostic_js_1.X12DiagnosticLevel.Error, errorMessage, seg.range));
            }
            ...
            this._transaction = new X12Transaction_js_1.X12Transaction(this._options);
    ...
    _processIEA(interchange, segment) {
        ...
        if (parseInt(segment.valueOf(1) ?? "") !== interchange.functionalGroups.length) {
            ...
            this.diagnostics.push(new X12Diagnostic_js_1.X12Diagnostic(X12Diagnostic_js_1.X12DiagnosticLevel.Error, errorMessage, segment.elements[0].range));
        }
        if (segment.valueOf(2) !== interchange.header.valueOf(13)) {
            ...
            this.diagnostics.push(new X12Diagnostic_js_1.X12Diagnostic(X12Diagnostic_js_1.X12DiagnosticLevel.Error, errorMessage, segment.elements[1].range));
        }
    }
