...
class X12QueryEngine {
    ...
    constructor(parser = true, mode = "strict") {
        ...
        this._parser = typeof parser === "boolean" ? new X12Parser_js_1.X12Parser(parser) : parser;
        this._mode = mode;
    }
    /**
    ...
    query(rawEdi, reference, defaultValue = null) {
        ...
        for (const group of interchange.functionalGroups) {
            for (const txn of group.transactions) {
                ...
                if (elmRefMatch === null) {
                    throw new Errors_js_1.QuerySyntaxError("Element reference queries must contain an element reference!");
                }
        ...
        for (const segment of segments) {
            ...
            if (this._testQualifiers(transaction, segment, qualifiers)) {
                if ((typeof value !== "undefined" && value !== null)) {
                    ...
                    results.push(new X12QueryResult(interchange, functionalGroup, transaction, segment, new X12Element_js_1.X12Element(), undefined));
                }
            }
        }
        return results;
